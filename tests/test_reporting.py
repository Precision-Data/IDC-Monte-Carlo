...
def test_write_all_outputs_creates_expected_files(sim_artifacts, tmp_path) -> None:
    ...
    # Severity weighting is enabled in plan v2.0; manuscript Table 3
    # CSV and source-of-truth Parquet are both produced.
    assert (out_dir / "tables" / "severity_weighted.csv").is_file()
    assert (out_dir / "severity_weighted.parquet").is_file()
    # v2.0 regime contrast statistic (Section 7.2)
    assert (out_dir / "tables" / "regime_contrast.csv").is_file()
...
def test_run_log_records_every_output_hash(sim_artifacts, tmp_path) -> None:
    ...
    # v2.0: 3 csv tables (primary, hospital, regime_contrast) +
    # 2 json status files (robustness, severity status) +
    # 1 severity csv + 1 severity parquet + 2 figures = 9 attached
    assert len(sim_artifacts.run_log["outputs"]) == log_before_count + 9
